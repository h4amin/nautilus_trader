...
# Init session state
for k, v in {"price":108000.0,"imbalance":0.0,"prob_drop":0.5,"balance":100000.0,"trades":[],"price_history":[108000.0]}.items():
    if k not in st.session_state: st.session_state[k] = v

placeholder = st.empty()

# Real BTC price (no key, no websockets)
def get_price():
    try:
        r = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd", timeout=3)
        return r.json()["bitcoin"]["usd"]
    except:
        return st.session_state.price * (1 + np.random.uniform(-0.0005,0.0005))
...
if len(st.session_state.price_history)>1200: st.session_state.price_history = st.session_state.price_history[-1200:]
...
if prob_drop>0.78 and np.random.rand()>0.33:
    ...
    if len(st.session_state.trades)>30: st.session_state.trades = st.session_state.trades[:30]
