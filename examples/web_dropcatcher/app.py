...
# Init
defaults = {
    "price": 108000.0, "imbalance": 0.0, "prob_drop": 0.5,
    "balance": 100000.0, "trades": [], "price_history": [108000.0]
}
...
# Get real BTC price (free, no key)
def get_btc_price():
    try:
...
# Simulated imbalance (you’ll see real swings)
imbalance = np.random.uniform(-0.9, 0.3) + 0.4 * np.sin(len(st.session_state.price_history) / 10)
...
st.sidebar.success("Auto-refresh: 0.5s | Real BTC price | No websockets = 100% stable")
