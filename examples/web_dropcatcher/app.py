# app.py — FINAL ZERO-FLASH (st.fragment + 1s Updates, Backtest + Live)
...
st.set_page_config(page_title="Nautilus Pro • Flash-Free", layout="wide", initial_sidebar_state="expanded")

# ZERO-FLASH THEME + CLEAN FONTS
st.markdown("""
<style>
...
with st.sidebar:
    ...
    mode = st.radio("Mode", ["Live (1s Update)", "Backtest"], index=0)
    base_leverage = st.slider("Leverage", 10, 125, 30)
    risk_pct = st.slider("Risk %", 0.5, 5.0, 2.0, 0.1)
    st.caption("OKX • Flash-Free • 2025")

# === LIVE MODE — FLASH-FREE WITH @st.fragment ===
if mode == "Live (1s Update)":
    st.title("OKX LIVE • Flash-Free Dashboard")

    @st.fragment(run_every=1.0)  # 1s isolated re-run (no full-script flash)
    def live_fragment():
        ...
        # DASHBOARD IN FRAGMENT (ISOLATED, NO FLASH)
    ...
    live_fragment()  # Run the fragment
